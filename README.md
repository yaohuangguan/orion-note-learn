...
Orion 是一款本地优先的开源笔记与学习工具，把富文本、手写画板、AI 整理和间隔复习放在同一个安静的工作台里。面向电脑、iPad 和手机浏览器，支持简体中文与 English 界面。
...
- 设置中可即时切换简体中文 / English，语言偏好会保存在当前浏览器；英文模式下 AI 学习伙伴也会用英文回答。
...
首次启动云服务前，把 `.dev.vars.example` 复制为 `.dev.vars` 并设置至少 32 个随机字符的 `AUTH_PEPPER`，再运行 `npm run cloud:migrate:local`。打开 **http://localhost:5173**；开发命令会同时启动 Vite、AI 服务和本地 Worker。不配置云账户也可以编辑笔记、手写、导出、手动制作闪卡及复习。
